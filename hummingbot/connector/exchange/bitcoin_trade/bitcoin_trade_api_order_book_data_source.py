...
from hummingbot.connector.exchange.bitcoin_trade.bitcoin_trade_order_book import BitcoinTradeOrderBook
...
class BitcoinTradeAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str]) -> Dict[str, float]:
        ...
        for trading_pair in trading_pairs:
            async with aiohttp.ClientSession() as client:
                while True:
                    response = await client.get(ROOT_PUBLIC_REST_URL + f'{convert_to_exchange_trading_pair(trading_pair)}/ticker')
                    if response.status == 429 or response.status == 502:
                        await asyncio.sleep(FAILURE_WAITING_TIME)
                        continue
                    if response.status != 200:
                        raise IOError(f"Error fetching Bitcoin Trade markets information. HTTP status is {response.status}.")
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str) -> Dict[str, Any]:
        async with client.get(f'{ROOT_PUBLIC_REST_URL}{convert_to_exchange_trading_pair(trading_pair)}/orders?limit={DEPTH}') as response:
            ...
            if response.status != 200:
                raise IOError(f"Error fetching Bitcoin Trade market snapshot for {trading_pair}. HTTP status is {response.status}.")
    ...
    async def _listen_trades_for_pair(self, pair: str, output: asyncio.Queue):
        ...
        url_path = ROOT_PUBLIC_REST_URL + f'{convert_to_exchange_trading_pair(pair)}/trades' + f'?page_size={PAGE_SIZE}'
                ...
                try:
                    async with client.get(url_path) as response:
                        ...
                        if response.status == 429 or response.status == 502:
                            ...
                            continue
                        if response.status != 200:
                            raise IOError(f'Error fetching BitcoinTrade market snapshot for {pair}. HTTP status is {response.status}.')
    ...
    async def _listen_order_book_for_pair(self, pair: str, output: asyncio.Queue = None):
        # there is no ws connection in the exchange api
        # this code emulates ws connection
        url_path = ROOT_PUBLIC_REST_URL + f'{convert_to_exchange_trading_pair(pair)}/orders/?limit={DEPTH}'
        async with aiohttp.ClientSession() as client:
            while True:
                try:
                    async with client.get(url_path) as response:
                        ...
                        if response.status == 429 or response.status == 502:
                            ...
                            continue
                        if response.status != 200:
                            raise IOError(f'Error fetching BitcoinTrade order book diffs for {pair}. HTTP status is {response.status}.')
