...
from hummingbot.connector.exchange.bitcoin_trade.bitcoin_trade_auth import BitcoinTradeAuth
from hummingbot.connector.exchange.bitcoin_trade.bitcoin_trade_in_flight_order import BitcoinTradeInFlightOrder
from hummingbot.connector.exchange.bitcoin_trade.bitcoin_trade_order_book_tracker import BitcoinTradeOrderBookTracker
from hummingbot.connector.exchange.bitcoin_trade.bitcoin_trade_utils import (
...
cdef class BitcoinTradeExchangeTransactionTracker(TransactionTracker):
    cdef:
        BitcoinTradeExchange _owner

    def __init__(self, owner: BitcoinTradeExchange):
        super().__init__()
        self._owner = owner
...
cdef class BitcoinTradeExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._bitcointrade_auth = BitcoinTradeAuth(api_key=bitcoin_trade_api_key)
        ...
        self._tx_tracker = BitcoinTradeExchangeTransactionTracker(self)
    ...
    cdef object c_get_fee(self,
        ...
        return estimate_fee("bitcoin_trade", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        self._trading_rules['BTC-BRL'] = TradingRule(
            trading_pair='BRLBTC',
        ...
        self._trading_rules['ETH-BRL'] = TradingRule(
            trading_pair='BRLETH',
        ...
        self._trading_rules['LTC-BRL'] = TradingRule(
            trading_pair='BRLLTC',
        ...
        self._trading_rules['BCH-BRL'] = TradingRule(
            trading_pair='BRLBCH',
        ...
        self._trading_rules['DAI-BRL'] = TradingRule(
            trading_pair='BRLDAI',
        ...
        self._trading_rules['EOS-BRL'] = TradingRule(
            trading_pair='BRLEOS',
        ...
        self._trading_rules['XRP-BRL'] = TradingRule(
            trading_pair='BRLXRP',
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Bitcoin Trade. "
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy LIMIT order to Bitcoin Trade for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to Bitcoin Trade. The order price is greater than or equal to the lowest selling price in the market."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting sell LIMIT order to Bitcoin Trade for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to Bitcoin Trade. The order price is less than or equal to the highest buy price in the market."
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
        except Exception:
